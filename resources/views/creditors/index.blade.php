<x-layout>
    <div class="grid grid-cols-profiles auto-rows-fr gap-4">
        ...
        <a href="{{ route( 'creditors.show', [ 'creditor' => $creditor ] ) }}" class="p-8 bg-white shadow rounded-2xl min-h-48">
            <h2 class="font-bold text-2xl text-slate-600 mb-2">{{ $creditor->name }}</h2>
            <div>
        ...
        <div class="p-8 border-8 border-dashed border-slate-600 rounded-2xl min-h-48 grid place-items-center overflow-hidden">
            <a href="{{ route('creditors.create') }}" class="flex flex-col items-center gap-2 transition-transform hover:-translate-y-2 duration-300">
                ...
                <p class="text-slate-600 text-2xl font-medium">Add creditor</p>
            </a>
        </div>
    </div>
</x-layout>
